...
public class LogPrinterImpl implements LogPrinter {
    ...
    @Override
    public void setLogFileDirName(@Nullable String dirName) {
        if (TextUtils.isEmpty(dirName)) {
            return;
        }
        LOG_FLODER_NAME = dirName;
        log(Log.ERROR, getTag(), "LOG_DIR_NAME:" + dirName, null);
    }
                ...
                try {
                    ...
                    if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                        ...
                        if (targetFile.length() < 1) {
                            if (application !=null)raf.write(getDeviceInfo(application).getBytes());
                        }
